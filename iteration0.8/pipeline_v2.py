...
class DataGenerator:
    ...
    def generate_causal_data(self):
        ...
        # 生成标签：基于压力强度（1Hz），然后扩展到50Hz
        # 强度 < 45: 舒适 (1.0)
        # 强度 45-65: 一般 (0.5)
        # 强度 > 65: 不舒适 (0.0)
        pressure_1hz = pressure[::self.fs]
        labels_1hz = np.where(pressure_1hz < 45, 1.0,
                             np.where(pressure_1hz < 65, 0.5, 0.0))
        # 将1Hz标签扩展到50Hz
        labels = np.repeat(labels_1hz, self.fs)[:len(pressure)]

        return self.t, pressure, t_h, hr_1hz, labels
...
class Pipeline:
    ...
    def run(self, use_existing_data=False):
        """运行完整Pipeline"""
        print("=" * 60)
        print("Iteration 2.5: 多模态时序对齐 → 特征 → 融合 → 模型")
        print("=" * 60)
        ...
        features_df.to_csv('features/fused_features.csv', index=False)
    ...
    def evaluate_test_data(self):
        ...
        print(f"\n  测试结果已保存: results/test_results.csv")
...
def main():
    ...
    features_df, metrics, importance = pipeline.run(use_existing_data=True)
    ...
    plt.tight_layout()
    plt.savefig('results/v2_visualization.png', dpi=150, bbox_inches='tight')
    plt.show()

    print("\n" + "=" * 60)
    print("Pipeline V2 完成！（心率特征增强版）")
    print("=" * 60)
    print("\n输出文件:")
    print("  - features/fused_features_v2.csv: 融合特征矩阵(V2)")
    print("  - results/test_results_v2.csv: 测试结果(V2)")
    print("  - results/v2_visualization.png: 可视化报告(V2)")
