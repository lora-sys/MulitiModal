...
class RegressionDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, npz_path):
        ...
        # 直接存储为tensor，Embedding层需要LongTensor
    ...
    def __getitem__(self, idx):
        return {
            ...
            'constitution': self.constitution[idx],  # LongTensor for Embedding
...
def main():
    """主函数"""
    print("=" * 60)
    print(f"🚀 {N_FOLDS}-Fold 交叉验证 (回归任务 - 快速版 V2)")
    print(f"模型: {MODEL_TYPE}")
    print(f"Epochs: {NUM_EPOCHS} (加速版)")
    print(f"随机种子: {RANDOM_SEED}")
    print("=" * 60)
    
    # 设备 - 强制使用CPU以避免GPU进程终止问题
    device = torch.device("cpu")
    print(f"[*] 使用设备: {device} (强制CPU模式)")
