...
def generate_enhanced_final_dataset(
    output_path='experiment/model/unified_dataset_enhanced_final.npz',
    test_output_path='experiment/model/test_dataset_enhanced_final.npz',
    n_samples_per_class=2500,  # 每类2500，总计10000样本
    ...
    # [4/5] 采样（只使用标签1,2,3的真实数据）
    print(f"\n[4/5] 采样 {n_samples_per_class * 3} 人 (每标签 {n_samples_per_class} 人)...")
    ...
    for label in [1, 2, 3]:  # 只处理标签1,2,3
        ...
        to_sample = min(available, n_samples_per_class)
...
if __name__ == "__main__":
    # 生成最终增强版数据集
    result = generate_enhanced_final_dataset(
        output_path='experiment/model/unified_dataset_enhanced_final.npz',
        test_output_path='experiment/model/test_dataset_enhanced_final.npz',
        n_samples_per_class=2500,  # 每类2500人
        test_ratio=0.15,
        random_seed=42
    )
    print(f'\n✅ 最终增强版数据集已生成!')