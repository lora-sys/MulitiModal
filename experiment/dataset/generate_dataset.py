...
def generate_dataset(
    output_path='experiment/model/unified_dataset_1000.npz',
    ...
    # [4/6] 采样
    print(f"\n[4/6] 采样 {n_samples_per_class * 4} 人 (每标签 {n_samples_per_class} 人)...")
    
    n_samples = n_samples_per_class * 4
    ...
    for label in range(4):
        if label == 0:
            # 标签0: 真实数据缺失，只使用波形库生成虚拟用户
            print(f"      标签 0: 真实数据缺失，从波形库生成 {n_samples_per_class} 个虚拟用户")
            
            for i in range(n_samples_per_class):
                # 虚拟用户特征（基于波形库的平均特征）
                age = np.random.uniform(25, 45)
                bmi = np.random.uniform(20, 28)
                hr = np.random.uniform(70, 90)
                spo2 = np.random.uniform(90, 95)
                health_score = np.random.uniform(50, 65)
                diagnosis_score = np.random.uniform(65, 80)
                
                # 随机体质
                const_id = np.random.randint(0, 38)
                
                static_basic_arr[idx] = [age, bmi, spo2, hr]
                static_scores_arr[idx] = [health_score, diagnosis_score]
                constitution_arr[idx] = const_id
            ...
            if available < n_samples_per_class:
                # 真实数据不足，调整采样数量
                to_sample = available
                print(f"      标签 {label}: 真实数据不足，采样 {to_sample} 人 (原始: {available} 人)")
            else:
                to_sample = n_samples_per_class
